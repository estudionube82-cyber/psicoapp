...
function getPlanLimits(plan) {
  return { free:{dias:15,whatsapp:20,informesIA:1}, pro:{dias:null,whatsapp:100,informesIA:3}, max:{dias:null,whatsapp:250,informesIA:25} }[plan] || {dias:15,whatsapp:20,informesIA:1};
}
...
function puedeUsar(feature) {
  ...
  if (plan === 'free' && lim.dias !== null && diasDesdeInicio(s.fechaInicio) >= lim.dias) return false;
  if (feature === 'whatsapp')   return (usos.whatsapp   || 0) < (lim.whatsapp + (ext.whatsapp || 0));
  if (feature === 'informesIA') return (usos.informesIA || 0) < lim.informesIA;
  return true;
}

function registrarUso(feature) {
  const s = getSuscripcion();
  if (!s || !puedeUsar(feature)) return false;
...
function avisoLimite(feature) {
  ...
  if (feature === 'whatsapp') {
    ...
    if (r <= 0)  return '🚫 Sin mensajes WhatsApp disponibles';
  ...
  if (feature === 'informesIA') {
    ...
    if (r <= 0)  return '🚫 Sin informes IA disponibles';
